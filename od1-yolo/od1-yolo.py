...
# draw all results
def draw_boxes_live(image, v_boxes, v_labels, v_scores):
    ...
    for i in range(len(v_boxes)):
        ...
        cv2.rectangle(image, (x1,y1), (x1+width,y1+height),(255,0,0), 1)
...
def load_and_predict_live():
    ...
    while (True):
        # define our new photo
        _,image_capt = cap.read()
        image_capt = cv2.resize(image_capt,(256,256))
        # convert to numpy array
        image = img_to_array(image_capt)
        # scale pixel values to [0, 1]
        image = image.astype('float32')
        image /= 255.0
        # add a dimension so that we have one sample
        image = expand_dims(image, 0)
        # load and prepare image
        image_w, image_h = cap.get(cv2.CAP_PROP_FRAME_WIDTH) ,cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
        ...
        for i in range(len(v_boxes)):
            print(v_labels[i], v_scores[i])
        # draw what we found
        draw_boxes_live(image_capt, v_boxes, v_labels, v_scores)
